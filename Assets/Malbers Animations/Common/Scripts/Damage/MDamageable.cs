...
namespace MalbersAnimations
{
    ...
    public class MDamageable : MonoBehaviour, IMDamage
    {
        ...
        protected void Start()
        {
            ...
            //CustomPatch: added assert for validating if the root points to itself (certain possible setups can accidentally lead to this)
            // Debug.Assert(Root != this, "MDamageable: Root should not point to itself. Please set the correct Owner of this Damageable", this);
        }
        ...
        {
            if (string.IsNullOrEmpty(name) || string.Equals(name, "default", System.StringComparison.OrdinalIgnoreCase)) //CustomPatch: optimized string compare
            {
                Profile_Restore();
            }
            else
            ...
            bool isCritical, Reaction2 customReaction, bool pureDamage, StatElement element, bool missed)
        {
            ...
            LastDamage = new DamageData(Direction, Damager, gameObject, damage, isCritical, statElement, DamageReaction, missed);
        ...
        public virtual void ReceiveDamage(StatID stat, float amount)
        {
            ...
            ReceiveDamage(Vector3.forward, Vector3.forward, null, modifier, false, null, false, null, false);
        }
