    ...
    public partial class MWeaponManager
    {
       ...
       // public bool SmoothEquip;
        ...
        protected virtual void OnEnable()
        {
            ...
            if (!Rider.IsUnityRefNull()) //CustomPatch: corrected null check for unity object interface type
            {
            ...
            if (!MInput.IsUnityRefNull()) ConnectInput(MInput, true);                //Connect the inputs from the Input Source //CustomPatch: corrected null check for unity object interface type
        ...
        protected virtual void OnDisable()
        {
            ...
            //CustomPatch: corrected null check for unity object interface types
            if (!Rider.IsUnityRefNull()) Rider.RiderStatus -= GetRiderStatus;    //Disconnect the notifications from the Rider 
            if (!MInput.IsUnityRefNull()) ConnectInput(MInput, false);           //Disconnect the inputs from the Input Source
        ...
        {
            WeaponType = 0;
            if (Weapon != null) //CustomPatch: corrected null check for unity object
                Weapon.ResetWeapon();
        ...
        {
            if (!Rider.IsUnityRefNull() && Weapon != null) //CustomPatch: corrected null check for unity object interface type
            {
        ...
        public virtual bool Aim
        {
            protected set
            {
                ...
                if (aim != value)
                {
                    ...
                    //Let know the Rider is Aiming. So if is using Straight Spine, it stops. (REVIEW)!!!!!!!!!***
                    if (!Rider.IsUnityRefNull()) Rider.IsAiming = value; //CustomPatch: corrected null check for unity object interface type
        ...
        public virtual void ExitAim()
        {
            ...
            if (!this.MInput.IsUnityRefNull()) //CustomPatch: corrected null check for unity object interface type
        ...
        public virtual void Equip_Weapon()
        {
            ...
            if (IsRiding && !Rider.IsUnityRefNull()) //CustomPatch: corrected null check for unity object interface type
            {
        ...
        private IEnumerator SwapWeaponsHolster(int HolsterToSwap)
        {
            if (Weapon)
            {
                ...
                while (WeaponAction == Weapon_Action.Aim) yield return null;    // Wait for the weapon is Unequipped Before it can Draw Another
