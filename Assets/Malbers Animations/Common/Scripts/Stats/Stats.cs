...
#pragma warning restore CS0414 // Add readonly modifier
    ...
    public class Stat
    {
        ...
        public virtual void Modify(float newValue, float time)
        {
            if (!IsImmune && Active)
            {
                Owner.StartCoroutine(ref I_ModifySlow, C_SmoothChangeValue(newValue, time));
                SetInmune();
            }
        }
        ...
        public virtual void Modify(float newValue, int ticks, float timeBetweenTicks)
        {
            ...
            Owner.StartCoroutine(ref I_ModifyPerTicks, C_ModifyTicksValue(newValue, ticks, timeBetweenTicks));
        }
        ...
        protected virtual void SetInmune()
        {
            if (ImmuneTime > 0)
            {
                if (Owner != null && Owner.enabled && Owner.gameObject.activeInHierarchy)
                    Owner.StartCoroutine(ref I_IsInmune, C_InmuneTime());
            }
        }
        ...
        protected virtual void StartRegeneration()
        {
            ...
            Owner.StartCoroutine(ref I_Regeneration, C_Regenerate());
        }
        ...
        protected virtual void StartDegeneration()
        {
            ...
            Owner.StartCoroutine(ref I_Degeneration, C_Degenerate());
        }
