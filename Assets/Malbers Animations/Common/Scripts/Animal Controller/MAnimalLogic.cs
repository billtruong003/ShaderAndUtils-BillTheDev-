...
namespace MalbersAnimations.Controller
{
    public partial class MAnimal
    {
        ...
        {
            Pivot_Hip = pivots.Find(item => string.Equals(item.name, "HIP", System.StringComparison.OrdinalIgnoreCase));  //CustomPatch: optimized string compare logic (.NET default "==" method is much slower when comparing strings)
            Pivot_Chest = pivots.Find(item => string.Equals(item.name, "CHEST", System.StringComparison.OrdinalIgnoreCase)); //CustomPatch: optimized string compare logic (.NET default "==" method is much slower when comparing strings)
        ...
        internal virtual void AlignRayCasting(float distance = 0)
        {
            ...
            if (Physics.Raycast(Main_Pivot_Point - DeltaPlatformPos, -Up, out hit_Chest, distance, GroundLayer, QueryTriggerInteraction.Ignore))
            {
                //CustomPatch: optimized relative expensive repeated fetch of the collider from the ray hit structure (the collider reference result is not actually cached in the struct :P)
            ...
            {
                var hipPoint = Pivot_Hip.World(t) - DeltaPlatformPos;// + DeltaVelocity;
                ...
                {
                    var hitHipCollider = hit_Hip.collider; //CustomPatch: optimized relative expensive repeated fetch of the collider from the ray hit structure (the collider reference result is not actually cached in the struct :P)
        ...
        internal Vector3 StoredGravityVelocity()
        {
            ...
            //CustomPatch: minor optimization for PC, but one that counts for every CPU cycle on mobile devices (always prefer multiplication instead of division when possible)
