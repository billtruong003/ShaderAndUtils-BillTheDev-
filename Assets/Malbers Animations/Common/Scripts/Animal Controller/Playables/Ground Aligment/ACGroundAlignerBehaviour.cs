...
namespace MalbersAnimations.Controller
{
    public class ACGroundAlignerBehaviour : PlayableBehaviour
    {
        ...
        public void GroundRayCast(MAnimal animal)
        {
            ...
            Vector3 MainPoint = Main_Pivot_Point(animal, 0); //Get the Main Pivot Position (Chest Pivot by default)
            ...
            if (HasHipPivot)
            {
                var hipPoint = Main_Pivot_Point(animal, 1); //Get the Hip Pivot Position
        ...
        public Vector3 Main_Pivot_Point(MAnimal animal, int pivotIndex)
        {
            return animal.pivots[pivotIndex].World(animal.transform);
        }
    }
}
