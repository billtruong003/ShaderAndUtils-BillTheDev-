        ...
        void DisplayHandles()
        {
            ...
            if (roadBumpPathGen.extrudePathPosList.Count > 1)
            {
                ...
                for (var i = 0; i < roadBumpPathGen.shapePosList.Count; i++)
                {
                    ...
                    #else
                    var fmh_266_190_638908046301924097 = Quaternion.identity; newTargetPosition = Handles.FreeMoveHandle(roadBumpPathGen.extrudePathPosList[0] - leftDir * roadBumpPathGen.shapePosList[i].x + up * roadBumpPathGen.shapePosList[i].y, size * .1f, Vector3.zero, Handles.SphereHandleCap);
                    #endif
