    ...
    public class MeshGenEditor : Editor
    {
        ...
        void UVFaceViewHandles()
        {
            ...
            if (meshGen.pathPosList.Count > 1)
            {
                ...
                for (var i = 0; i < meshGen.shapePosList.Count; i++)
                {
                    ...
                    #else
                    var fmh_662_162_638908046302540550 = Quaternion.identity; newTargetPosition = Handles.FreeMoveHandle(meshGen.pathPosList[0] - leftDir * meshGen.shapePosList[i].x + upDir * meshGen.shapePosList[i].y, size * .1f, Vector3.zero, Handles.SphereHandleCap);
                    #endif
                }
            }
        #endregion
        ...
        #region
            ...
            if (meshGen.pathPosList.Count > m_endPathPos.intValue)
            {
                ...
                #else
                var fmh_682_86_638908046302544386 = Quaternion.identity; Handles.FreeMoveHandle(meshGen.pathPosList[m_startPathPos.intValue], size * .1f, Vector3.zero, Handles.SphereHandleCap);
                var fmh_683_84_638908046302547366 = Quaternion.identity; Handles.FreeMoveHandle(meshGen.pathPosList[m_endPathPos.intValue], size * .1f, Vector3.zero, Handles.SphereHandleCap);
                #endif
