Shader "Nature/URP Tree Creator Leaves"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 LitPassFragment(Varyings input) : SV_Target
            {
                ...
                surfaceData.normalTS = UnpackNormalmapRGorAG(SAMPLE_TEXTURE2D(_BumpMap, sampler_BumpMap, input.uv));
                surfaceData.occlusion = 1.0h;
                surfaceData.alpha = alpha;

                float3 normalWS = TransformTangentToWorld(surfaceData.normalTS,
                    half3x3(input.tangentWS, input.bitangentWS, input.normalWS));

                Light mainLight = GetMainLight();
                half3 attenuatedLightColor = mainLight.color * mainLight.shadowAttenuation;
                half3 diffuse = surfaceData.albedo * LightingLambert(attenuatedLightColor, mainLight.direction, normalWS);
