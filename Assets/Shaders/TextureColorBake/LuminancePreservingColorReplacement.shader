Shader "ShaderAndUtils/Utility/AdvancedColorReplacement_Bake"
{
    Properties
    {
        [Header(Main Texture)]
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 convertSrgbToLinear(float3 srgb)
            {
                ...
                float3 gammaPart = pow(abs(srgb + 0.055) / 1.055, 2.4);
            ...
            float3 convertSrgbToLab(float3 srgb)
            {
                ...
                return convertXyzToLab(convertLinearToXyz(linearRgb));
            }
            ...
            float4 frag(Varyings input) : SV_Target
            {
                ...
                float3 originalColorLinear = convertSrgbToLinear(originalColorSrgb.rgb);
                
                float3 originalLab = convertLinearToLab(originalColorLinear);
                ...
                float3 replacementColorLinear = convertSrgbToLinear(_ReplacementColor.rgb);
                
                float3 finalRgbLinear = lerp(originalColorLinear, replacementColorLinear, mask);
                
                return float4(finalRgbLinear, originalColorSrgb.a);
            }
            ENDHLSL
        }
    }
