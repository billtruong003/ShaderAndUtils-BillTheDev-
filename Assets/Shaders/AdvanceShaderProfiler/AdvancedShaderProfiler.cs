...
public class ProfessionalShaderProfiler : EditorWindow
{
    ...
    private static MethodInfo _getPassNameMethod;
    ...
    [MenuItem("Window/Analysis/Professional Shader Profiler")]
    ...
    {
        if (_getPassNameMethod == null || _getPassLightModeMethod == null) return;
        profile.Passes.Clear();
        for (int i = 0; i < profile.Shader.passCount; i++)
        {
            string passName = (string)_getPassNameMethod.Invoke(null, new object[] { profile.Shader, i });
            string lightMode = (string)_getPassLightModeMethod.Invoke(null, new object[] { profile.Shader, i });
    ...
    {
        if (_getPassNameMethod != null) return;
        Type shaderUtilType = typeof(ShaderUtil);
        _getPassNameMethod = shaderUtilType.GetMethod("GetPassName", BindingFlags.Static | BindingFlags.NonPublic);
        _getPassLightModeMethod = shaderUtilType.GetMethod("GetPassLightMode", BindingFlags.Static | BindingFlags.NonPublic);

        if (_getPassNameMethod == null || _getPassLightModeMethod == null)
        {
            Debug.LogError("ProfessionalShaderProfiler: Could not find internal ShaderUtil methods via reflection. Pass analysis will be disabled. This may be due to a Unity version update.");
        }
    }
    #endregion
}
