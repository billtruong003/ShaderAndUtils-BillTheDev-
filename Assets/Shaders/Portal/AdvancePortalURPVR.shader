Shader "PRO/AdvancedPortalURP_VRCutout"
{
    Properties
    {
        // Các Properties giữ nguyên như phiên bản VR Optimized
        [Header(Portal Core)]
    ...
    SubShader
    {
        // Thay đổi quan trọng nằm ở Tags
        Tags
        {
            "RenderType" = "TransparentCutout"
            "Queue" = "AlphaTest" // Hàng đợi chuẩn cho Cutout
        ...
        Pass
        {
            ...
            float4 frag(Varyings IN) : SV_Target
            {
                ...
                // portalMask bây giờ chỉ dùng cho clip() và tính toán màu
                float portalMask = calculateVisibilityMask(distortedRadius, _PortalRadius, _EdgeSoftness);
                clip(portalMask - 0.5); // Sử dụng ngưỡng 0.5 cho Cutout để có cạnh sắc nét
                ...
                // Không cần lerp hay alpha nữa, chỉ cần cộng màu
                float4 finalColor = coreColor + rimColor;
                finalColor.a = 1.0; // Là Opaque
