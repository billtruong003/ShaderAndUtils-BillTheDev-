Shader "CleanCode/OptimizedCosmicGlass"
{
    Properties
    {
        [Header(Performance and Quality Controls)]
        [Toggle(_PARALLAX_ON)] _EnableParallax("Enable Parallax (Depth)", Float) = 1
        [Toggle(_TRIPLANAR_ON)] _EnableTriplanar("Enable Triplanar (No Stretch)", Float) = 1
        
        [Header(Depth and Parallax)]
        _ParallaxDepth("Parallax Depth", Range(0, 0.1)) = 0.04
        _GalaxyDepthBias("Galaxy Brightness", Range(0, 2)) = 1.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Đây là phần tối ưu hóa quan trọng nhất
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                float3 posStars, posNoise1, posNoise2;
                #if defined(_PARALLAX_ON)
                    float3 parallaxOffset = viewDirWS * _ParallaxDepth;
                    posStars = IN.posWS + parallaxOffset * 2.0;
                    posNoise2 = IN.posWS + parallaxOffset * 1.0;
                    posNoise1 = IN.posWS + parallaxOffset * 0.5;
                #else
                    posStars = IN.posWS;
                    posNoise2 = IN.posWS;
                    posNoise1 = IN.posWS;
                ...
                half4 n1, n2; half s;
                #if defined(_TRIPLANAR_ON)
                    n1 = SampleTriplanar(TEXTURE2D_ARGS(_NoiseTex1,sampler_NoiseTex1),posNoise1,normalWS,_NoiseScale1,_NoiseScrollSpeed1);
                    n2 = SampleTriplanar(TEXTURE2D_ARGS(_NoiseTex2,sampler_NoiseTex2),posNoise2,normalWS,_NoiseScale2,_NoiseScrollSpeed2);
                    s  = SampleTriplanar(TEXTURE2D_ARGS(_StarsTex,sampler_StarsTex),posStars,normalWS,_StarsScale,_StarsScrollSpeed).r;
                #else // Fallback to cheaper Planar Mapping
                    n1 = SAMPLE_TEXTURE2D(_NoiseTex1,sampler_NoiseTex1, posNoise1.xz * _NoiseScale1 + _NoiseScrollSpeed1 * _Time.y);
                    n2 = SAMPLE_TEXTURE2D(_NoiseTex2,sampler_NoiseTex2, posNoise2.xz * _NoiseScale2 + _NoiseScrollSpeed2 * _Time.y);
                    s  = SAMPLE_TEXTURE2D(_StarsTex,sampler_StarsTex, posStars.xz * _StarsScale + _StarsScrollSpeed * _Time.y).r;
                #endif
