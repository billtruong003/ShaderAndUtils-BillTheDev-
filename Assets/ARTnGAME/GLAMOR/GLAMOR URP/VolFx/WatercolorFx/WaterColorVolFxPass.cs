...
namespace Artngame.GLAMOR.VolFx
{
    ...
    public class WaterColorVolFxPass : VolFxProc.Pass
    {        
        ...
        WaterColorVolFxVol settings;

        public override bool Validate(Material mat)
        {
            if (settings == null)
            {
                settings = Stack.GetComponent<WaterColorVolFxVol>();
            }
            ...
            , RenderTextureDescriptor descA, bool isRG)
        {
            ...
            if (_materialA == null)
            {
                _materialA = Resources.Load<Material>("WaterColorVolFx");
            }
            if (_materialA == null)
            {
                _materialA = new Material(Resources.Load<Shader>("WaterColorVolFx"));
            }
           ...
           // RenderTextureDescriptor descB = desc;
            ...
            _materialA.SetColor("_FillColor", settings.fillColor.value);
            _materialA.SetColor("_EdgeColor", settings.edgeColor.value);
            _materialA.SetVector("_EffectParams", new Vector4(
                settings.edgeContrast.value,
                settings.blurWidth.value,
                Mathf.Exp(settings.blurFrequency.value - 0.5f) * 6f,
                settings.hueShift.value));
            _materialA.SetFloat("_Interval", settings.interval.value);
            _materialA.SetInt("_Iteration", settings.iteration.value);

            cmd.SetGlobalTexture("_NoiseTex", settings._NoiseTex.value);
            cmd.SetGlobalTexture("_MainTexA", source);
            //cmd.Blit(source, destination, material);
            Utils.Blit(cmd, source, dest, _materialA, 13);
