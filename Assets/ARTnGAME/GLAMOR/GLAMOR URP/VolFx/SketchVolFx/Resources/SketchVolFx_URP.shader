			...
			*/
			...
			half4 Fragment(VaryingsA input) : SV_Target
			{
					...
					//#else
									...
									//color.a *= d;
									//float d = saturate((length(forward) / 5.0));// -_SphereSize) / 5.0);
									//color.a *= d * scaling.w + 1;
									//float d = saturate((length(pow(forward, 35)) / 15.0));// -_SphereSize) / 5.0);
									if (length(wposA - _WorldSpaceCameraPos) > 400 * scaling.w) {
										color.a *= 1-((length(wposA - _WorldSpaceCameraPos) - 400 * scaling.w) / length(wposA - _WorldSpaceCameraPos));
									}
