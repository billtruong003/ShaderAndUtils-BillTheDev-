namespace Artngame.GLAMOR.Sketch
{
    ...
    public class Sketch : ScriptableRendererFeature
    {
        ...
        class SketchRenderPass : ScriptableRenderPass
        {
            ...
            {
                var shader = Shader.Find("GIBLION/Sketch");

                if (shader == null)
                {
                    Debug.LogError("Cannot find shader: \"GIBLION/Sketch\".");
                    return;
                }
            ...
            private static void ExecuteMainPass(RasterCommandBuffer cmd, RTHandle source, Material material)//, RenderTexture shadows)
            {
                // Set Sketch effect properties.
                var settings = VolumeManager.instance.stack.GetComponent<SketchSettings>();
                material.SetTexture("_SketchTexture", settings.sketchTexture.value);
                ...
                material.SetVector("_CrossHatchingRotation", settings.crossHatchingRotation.value);
                ...
                // material.SetTexture("_ShadowmapTexture", (RenderTexture)Shader.GetGlobalTexture(Shader.PropertyToID("_ScreenSpaceShadowmapTexture")));
               // material.SetTexture("_ShadowmapTexture", shadows);
            ...
            public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
            {
                ...
                colorCopyDescriptor.depthBufferBits = 16;
                colorCopyDescriptor.depthStencilFormat = UnityEngine.Experimental.Rendering.GraphicsFormat.B8G8R8A8_UNorm;
                ...
                using (var builder = renderGraph.AddRasterRenderPass<MainPassData>("Sketch_MainPass", out var passData, profilingSampler))
                {
                    ...
                    // material.SetTexture("_ShadowmapTexture", (RenderTexture)Shader.GetGlobalTexture(Shader.PropertyToID("_ScreenSpaceShadowmapTexture")));
                     ...
                     builder.UseGlobalTexture(Shader.PropertyToID("_ScreenSpaceShadowmapTexture"));
                   ...
                   // builder.SetRenderAttachment(texture, 0, AccessFlags.Write);
                    ...
                    //https://docs.unity3d.com/6000.1/Documentation/Manual/urp/render-graph-import-a-texture.html
                    // RTHandle rtHandle = RTHandles.Alloc(texturePassedIn);
                   // TextureHandle textureToRead = renderGraph.ImportTexture(texture);
                    //  builder.UseTexture(textureToRead, AccessFlags.Read);
