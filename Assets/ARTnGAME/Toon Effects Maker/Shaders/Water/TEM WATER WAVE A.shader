    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float4 node_2081 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((o.uv0+node_2081.g*float2(0.25,0.11)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
...
/////// Vectors:
                ...
                float4 node_2081 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((i.uv0+node_2081.g*float2(0.25,0.11)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
...
////// Lighting:
				...
				UNITY_LIGHT_ATTENUATION(attenuation, i, i.posWorld.xyz); //v0.1 2018
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.tangentDir = normalize( mul( unity_ObjectToWorld, float4( v.tangent.xyz, 0.0 ) ).xyz );
                o.binormalDir = normalize(cross(o.normalDir, o.tangentDir) * v.tangent.w);
                float4 node_8609 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((o.uv0+node_8609.g*float2(0.25,0.11)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                v.vertex.xyz += (node_133*_BulgeScale*v.normal*(v.normal*_BulgeScale_copy));
                o.posWorld = mul(unity_ObjectToWorld, v.vertex);
                float3 lightColor = _LightColor0.rgb;
                o.pos = UnityObjectToClipPos(v.vertex);
...
/////// Vectors:
                ...
                float3 _Normals_var = UnpackNormal(tex2D(_Normals,TRANSFORM_TEX(i.uv0, _Normals)));
                float4 node_8609 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((i.uv0+node_8609.g*float2(0.25,0.11)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                float3 normalLocal = normalize(lerp(_Normals_var.rgb,float3(0,0,1),node_133));
                float3 normalDirection = normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float node_133 = pow((abs((frac((o.uv0+node_2083.g*float2(0.25,0.11)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                v.vertex.xyz += (node_133*_BulgeScale*v.normal*(v.normal*_BulgeScale_copy));
                o.pos = UnityObjectToClipPos(v.vertex);
...
/////// Vectors:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.uv0 = v.texcoord0;
                o.normalDir = mul(unity_ObjectToWorld, float4(v.normal,0)).xyz;
                float4 node_4131 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((o.uv0+node_4131.g*float2(0.25,0.11)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                v.vertex.xyz += (node_133*_BulgeScale*v.normal*(v.normal*_BulgeScale_copy));
                o.pos = UnityObjectToClipPos(v.vertex);
