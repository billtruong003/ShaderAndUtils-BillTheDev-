...
namespace Artngame.TEM {
	...
	public class LightFlashTEM : MonoBehaviour {	
		...
		void Update () {
			...
			if(emits){
				if(Time.fixedTime - start_emit_time <= emit_time){
					...
					this_light.intensity = lightPower*Random.Range(1,3)* noise.Noise(timex + this_light.intensity, timey + this_light.intensity, timez + this_light.intensity);
