...
void Outline_float(float2 UV, float OutlineThickness, float DepthSensitivity, 
	float NormalsSensitivity, float ColorSensitivity, float4 OutlineColor, float4 OutlineControls, out float4 Out)
{
...
void OutlineG_float(float2 UV, float OutlineThickness, float DepthSensitivity, float NormalsSensitivity, float ColorSensitivity, 
	float4 OutlineColor, float4 OutlineControls, out float4 Out)
{
	float halfScaleFloor = floor(OutlineThickness * 0.5);
	float halfScaleCeil = ceil(OutlineThickness * 0.5);
...
//RAINDROPS
float2 ComputeRipple(SamplerState sampler_Lux_RainRipples, float4x4 _CamToWorld, Texture2D _Lux_RainRipples, float4 UV, float CurrentTime, float Weight)
{
...
float3 AddWaterFlowRipples(SamplerState sampler_Lux_RainRipples, float4x4 _CamToWorld, Texture2D _Lux_RainRipples, float2 i_wetFactor, float3 i_worldPos, float2 lambda, float i_worldNormalFaceY, float fadeOutWaterBumps,
...
void RainDrops_float(
	...
	SamplerState sampler_Lux_RainRipples,
