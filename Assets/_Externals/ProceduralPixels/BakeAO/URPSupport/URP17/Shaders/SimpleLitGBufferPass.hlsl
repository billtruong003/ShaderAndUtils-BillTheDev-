...
Varyings LitPassVertexSimple(Attributes input)
{
    ...
    [flatten]
    if (_AOTextureUV == 0)
        output.aoTextureUV = input.texcoord;
    else if (_AOTextureUV == 1)
        output.aoTextureUV = input.staticLightmapUV;
    else if (_AOTextureUV == 2)
        output.aoTextureUV = input.dynamicLightmapUV;
...
FragmentOutput LitPassFragmentSimple(Varyings input)
{
    ...
    surfaceData.occlusion = occlusionValue;
    ...
    // BakeAO add begin
    inputData.bakedGI *= occlusionValue;
    // BakeAO add end
